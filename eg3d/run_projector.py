...
@click.option('--datajson', help='camera parameters path', type=str, required=True, metavar='STR', show_default=True)
...
def run(
    ...
    c = camera_dic[os.path.basename(image_path)]
    c = torch.FloatTensor(c).unsqueeze(0).cuda()
    print(c.shape)
