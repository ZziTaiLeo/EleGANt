...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        self.backbone = StyleGAN2Backbone(z_dim, c_dim, w_dim, img_resolution=256,
                                          img_channels=32*3, mapping_kwargs=mapping_kwargs, **synthesis_kwargs)
        self.superresolution = dnnlib.util.construct_class_by_name(
            class_name=rendering_kwargs['superresolution_module'], channels=32, img_resolution=img_resolution, sr_num_fp16_res=sr_num_fp16_res, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
        self.decoder = OSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get(
            'decoder_lr_mul', 1), 'decoder_output_dim': 32})
    ...
    def mapping(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False):
        print('c:', type(c))
        print('rendering_kwargs_get:', self.rendering_kwargs.get('c_scale', 0))
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        planes = planes.view(len(planes), 3, 32,
                             planes.shape[-2], planes.shape[-1])

        # Perform volume rendering
        feature_samples, depth_samples, weights_samples = self.renderer(
            planes, self.decoder, ray_origins, ray_directions, self.rendering_kwargs)  # channels last
...
class OSGDecoder(torch.nn.Module):
    ...
    def forward(self, sampled_features, ray_directions):
        ...
        x = self.net(x)
        x = x.view(N, M, -1)
        rgb = torch.sigmoid(x[..., 1:])*(1 + 2*0.001) - \
            0.001  # Uses sigmoid clamping from MipNeRF
        sigma = x[..., 0:1]
