...
import wandb
from eg3d import optimizer_config as global_config
import torch
import matplotlib.pyplot as plt


def log_image_from_w(w, G, c,name):
    img = get_image_from_w(w, G,c)
...
def log_images_from_w(ws, G, c,names):
    for name, w in zip(names, ws):
        w = w.to(global_config.device)
        log_image_from_w(w, G,c, name)


def plot_image_from_w(w, G,c):
    img = get_image_from_w(w, G,c)
    pillow_image = Image.fromarray(img)
...
def save_single_image(base_dir, image_latent, G, c,file_name):
    image_to_save = get_image_from_w(image_latent, G,c)
